...
impl Poll {
    ...
    pub fn poll(&mut self, events: &mut Events, timeout: Option<i32>) -> io::Result<()> {
        ...
        // calling epoll wait to know when the server has reponded
        let res = unsafe { ffi::epoll_wait(epoll_fd, events.as_mut_ptr(), max_events, timeout) };
        if res < 0 {
            return Err(io::Error::last_os_error());
        };
        unsafe {
...
impl Registry {
    pub fn register(&self, source: &TcpStream, token: usize, interests: i32) -> io::Result<()> {
        ...
        let res = unsafe { 
            // For the Epoll instance of File Descriptor self.raw_fd
            // Add (EPOLL_CTL_ADD) the following TcpStream FileDescriptors 
            // With the following interests (read) in any of them
